...
public interface Message extends Serializable, AttributeReadUuid, AttributeReadMessage, AttributeReadMessageCode {
   ...
   * <b>ATTENTION:</b><br>
   * This method is designed for server applications with {@link net.sf.mmm.util.nls.api.NlsMessage NLS}. On client side
   * (e.g. for GWT clients) only a single locale may be supported at a time and this method may behave like
   * {@link #getMessage()} ignoring the {@link Locale}.
   *
   ...
   * end-user (e.g. by providing invalid data). Unlike {@link #TYPE_VALIDATION_FAILURE} this is an actual error that has
   * been detected after a successful validation.</li>
   * <li>{@value #TYPE_TECHNICAL_ERROR} - for an {@link net.sf.mmm.util.exception.api.NlsThrowable error} related to a
   * technical problem (e.g. programming failure such as {@link NullPointerException} or operational error like
   * {@link java.net.ConnectException}).</li>
   * <li>{@value #TYPE_WARNING} - for a warning such as "The hit-list for your query has been truncated."). In such case
   * it should be possible to continue the use-case or operation normally.</li>
