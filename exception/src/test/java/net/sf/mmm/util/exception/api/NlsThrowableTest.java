...
public class NlsThrowableTest extends Assertions {
  ...
  protected void checkException(String expectedMessage, NlsThrowable e, Locale locale) {
    ...
    assertThat(stacktrace).contains(expected);
    LoggerFactory.getLogger(NlsThrowableTest.class).error("This is a test and should contain the UUID and code of the exception", e);
  }
  ...
  public void testNlsException() {
    ...
    NlsException e = new NlsException(NlsAccess.getFactory().create(MyExceptionResourceBundle.ERR_NULL, source)) {
  ...
  public void testNlsParseException() {
    ...
    assertThat(parseException.getNlsMessage().getLocalizedMessage(AbstractNlsMessage.LOCALE_ROOT))
        .isEqualTo("Failed to parse \"" + value + "\" from \"" + source + "\" as value of the type \"" + type.getName() + "\"!");
  }
  ...
  public void testNlsRuntimeException() {
    ...
    String source = "bad boy";
    NlsRuntimeException e = new NlsRuntimeException(NlsAccess.getFactory().create(MyExceptionResourceBundle.ERR_NULL, source)) {
