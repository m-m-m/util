 ...
 * implementation is based on spring but this small facade was created to support environments where no
 * springframework is available such as GWT (that does not even support {@link ThreadLocal}). In GWT contexts
 * a GWT compatible implementation is provided automatically.
 *
...
public final class UserSessionAccess {
  ...
  private static void setProvider(UserSessionProvider sessionProvder) {
    ...
    if (provider != null) {
      LoggerFactory.getLogger(UserSessionAccess.class).warn("ATTENTION: replacing existing UserSessionProvider {} with {}!", provider, sessionProvder);
    }
    synchronized (UserSessionAccess.class) {
  ...
  /**
   * @return the login of the current user (e.g. "john.doe"). Will be {@code null} if called outside the scope
   *         of a current user session.
   */
   ...
   */
  public abstract static class AbstractUserSessionProvider extends AbstractComponent implements UserSessionProvider {
