 ...
 * initial {@link net.sf.mmm.util.pojo.api.Pojo} reflectively.<br>
 ...
 * {@link #getPojoPath() String} with a specific syntax and semantic. <h3>Syntax
 * </h3> The syntax of a {@link PojoPath} is defined as follows:<br>
 * <code>{@link PojoPath} = &#171;Segment&#187; | &#171;{@link PojoPath}&#187;.&#171;Segment&#187;</code>
 * <br>
 ...
 * 
 * <h3>Semantic</h3> &#171;Property&#187; stands for a property of the
 * {@link net.sf.mmm.util.pojo.api.Pojo}.
 * <ul>
 * <li>If the actual {@link net.sf.mmm.util.pojo.api.Pojo} is a
 ...
 * {@link net.sf.mmm.util.pojo.descriptor.api.PojoDescriptor#getProperty(Object, String)
 * name of a property} of the actual {@link net.sf.mmm.util.pojo.api.Pojo}. If
 * the actual {@link net.sf.mmm.util.pojo.api.Pojo} does NOT have such property,
 ...
 * <ul>
 * <li>If the actual {@link net.sf.mmm.util.pojo.api.Pojo} is an array, the
 * &#171;Index&#187; represents the array-index to access a contained value.</li>
 * <li>If the actual {@link net.sf.mmm.util.pojo.api.Pojo} is a
 * {@link java.util.List}, the &#171;Index&#187; is used as the list-index to
 * {@link java.util.List#get(int) get} and
 ...
 * by the {@link PojoPathFunctionManager#getFunction(String) name of the
 * function}. {@link PojoPathFunction} can make assumptions on the actual
 * {@link net.sf.mmm.util.pojo.api.Pojo} they operate on. If the actual
 * {@link net.sf.mmm.util.pojo.api.Pojo} does NOT follow these assumptions the
 * {@link PojoPath} will be illegal and cause a {@link PojoPathException}.</li>
 * </ul>
 * 
 * <h3>Transitivity</h3> For two {@link PojoPath}-strings <code>path1</code> and
 * <code>path2</code> and for<br>
 * <code>
 * Object pojo;<br>
...
public interface PojoPath {
  ...
  /**
   * The character that separates the {@link #getSegment() segments} of a
   * {@link #getPojoPath() PojoPath}. The value ({@value} ) will never change.
   * It is NOT necessary to use this constant to construct a {@link PojoPath}.
   */
  char SEPARATOR = '.';
  ...
  /**
   * This method gets the last segment of this current {@link PojoPath}. E.g. if
   * this path represents <code>"foo.bar.property"</code> then this method would
  ...
  /**
   * This method gets the <em>index</em> given by the {@link #getSegment()
   * segment} or <code>null</code> if it is no index.<br>
   * An <em>index</em> is an integer that represents the position of an ordered
   * container (array or {@link java.util.List}).<br>
   * If a {@link #getSegment() segment} starts with a Latin digit, it is treated
   * as {@link #getIndex() index} and has to be a
   * {@link Integer#parseInt(String) valid} integer-value. However parsing
   ...
   *         <code>null</code> if it is no index.
   */
  Integer getIndex();

  /**
   * This method gets the name of the <em>function</em> given by the
   * {@link #getSegment() segment} or <code>null</code> if it is no function.<br>
   * The <em>function</em>-name
   * {@link PojoPathFunctionManager#getFunction(String) identifies} a
   * {@link PojoPathFunction} that will be used to evaluate the
   * {@link #getSegment() segment}.<br>
   * If a {@link #getSegment() segment} starts with the character
   * {@link PojoPathFunction#FUNCTION_NAME_PREFIX} (<code>'&#64;'</code>), it is
   * treated as {@link #getFunction() function}.
   * 
   * @return the {@link #getSegment() segment} excluding the first character or
   *         <code>null</code> if the {@link #getSegment() segment} does NOT
   *         start with {@link PojoPathFunction#FUNCTION_NAME_PREFIX}.
   */
  String getFunction();
