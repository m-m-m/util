...
/**
 * This enum contains the available styles for container types. A container type is an array,
...
public enum CliContainerStyle {
   ...
   * {@link CliOption#name() named} <code>--list</code> and with this style via
   * {@link CliOption#containerStyle()}. It may also have another boolean option named <code>--bar</code>. Now
   * we call the program like this:
   * 
   * <pre>
   * Foo --list first --bar --list "second value" --list 3
   * </pre>
   * 
   * So <code>listProperty</code> will be filled with a List containing {"first", "second value", "3"}.
   */
  MULTIPLE_OCCURRENCE,

  /**
   * The {@link CliOption} or {@link CliArgument} may occur only once. Multiple values are supplied as comma
   * separated list. In the above example call the program as following:
   * 
   * <pre>
