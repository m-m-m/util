 ...
 * <li>{@link #getTypeId() type-id} - is the ID of the type of the object identified by this {@link EntityId}
 * .</li>
 * <li>{@link #getRevision() revision} - a object can potentially have multiple revisions (as history of
 * changes). A {@link EntityId} uniquely identifies the specific {@link #getRevision() revision}.</li>
 * </ul>
...
public interface EntityId extends SimpleDatatype<String> {
  ...
  /**
   * This method gets the {@link net.sf.mmm.util.lang.api.attribute.AttributeReadId#getId() unique identifier}
   * of an {@link GenericEntity entity} (or any other object).
   * 
  ...
  /**
   * This method gets the identifier representing the <em>type</em> of the referenced {@link GenericEntity
   * entity} or object. The type is typically directly related to the {@link Class} reflecting the object.
   * However, in dynamically or mixed typed object-worlds (often used in content-management-systems) two
   * instances of the same java {@link Class} may be considered to have different types that can only be
   * determined at runtime. To resolve an {@link EntityId} a central component is required that can map the
   * {@link #getTypeId()} to the actual type (e.g. {@link Class} object) and vice versa.
   * 
   * @return the classId.
   */
  long getTypeId();
