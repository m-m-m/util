 ...
 * together with metadata that is detected while streaming the data.<br>
 ...
 * {@link java.io.InputStream} and {@link java.io.OutputStream}). A typical
 ...
 * file, analyze it and then transfer the file to the database.<br>
 * The {@link DetectorStream} allows you to get the metadata on the fly while
 ...
 * interface is the container for the wrapper stream.<br>
...
public abstract interface DetectorStream {
  ...
  /**
   * This method gets the context with the detected metadata. It is immutable
   * and should NOT be modified. This method should NOT be called, before this
   * detector stream is {@link #isDone() done}.<br>
  ...
  /**
   * This method determines if the detection is done. If the stream has been
   * processed to the end this method will always return <code>true</code>.
   * However the detection may be done before the end of the stream was reached
   * (e.g. because all {@link #getMetadata() metadata} comes from the header of
   * the data).
   * 
