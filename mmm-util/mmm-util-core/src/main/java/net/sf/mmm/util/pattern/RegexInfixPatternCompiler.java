 ...
 * <code>pattern</code> given as string in a way similar to GNU-utilities like
 * <code>sed</code> or <code>grep</code>. This means that if no leading "^"
 * (or ".*") or no trailing "$" (or ".*") is present, an according ".*" prefix
 * and suffix is added implicit. This causes that "\.xml$" matches "config.xml"
 * and "^/etc/" matches "/etc/passwd".<br>
 * <b>ATTENTION:</b><br>
