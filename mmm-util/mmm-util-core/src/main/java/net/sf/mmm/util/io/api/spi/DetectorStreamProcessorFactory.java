...
public interface DetectorStreamProcessorFactory {
  ...
  /**
   * This method determines the required number of bytes a {@link DetectorStreamProcessor} needs as lookahead
   * in order to make its decisions. The {@link net.sf.mmm.util.io.api.DetectorStream} will use the maximum
   ...
   * available. In that case he can consume all these bytes and make its decision. Otherwise he returns from
   * {@link DetectorStreamProcessor#process(DetectorStreamBuffer, java.util.Map, boolean)} and waits for the
   * next call that will guarantee the buffer is refilled with at least the specified lookahead.<br>
   * 
  ...
  /**
   * This method gets a {@link DetectorStreamProcessor} instance managed by this factory. Typically the
   * factory will create a new instance per {@link #createProcessor() request}. However if a
   * {@link DetectorStreamProcessor} is thread-safe the factory can always return the same instance.
   * 
