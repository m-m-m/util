 ...
 * representing a value of a specific type. It is typically <em>immutable</em>
 * so it gets its value assigned at construction time and can then not be
 * modified anymore. Further, a datatype should ensure validation so it is NOT
 * possible to create an instance with an inconsistent value. Based on the
 * {@link Datatype} a presentation layer can decide how to view and how to edit
 * the value. Therefore a structured data model should make use of custom
 * datatypes in order to be expressive.<br/>
 ...
 * reason (e.g. a Blob may allow to append data as it would be inefficient to
 * create a copy instead). In such case it is recommended to declare a view
 * interface that only declares methods to read (getters).<br/>
 ...
 * should have a {@link java.lang.reflect.Constructor} that is compatible with
 * {@link #getValue()}. It is suitable and also recommended to use the class
 * implementing the datatype as API omitting a dedicated interface if possible.
 * An {@link Enum} implementing this interface should also offer a static method
 * called <code>fromValue(V value)</code> that returns the appropriate
 * {@link Enum} instance or <code>null</code> if no such instance exists.
 * 
