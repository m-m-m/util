...
/**
 * A {@link CliArgument} is used to annotate a property (member variable of some class or setter-method) that
 * should be set from a main-program via a commandline argument. Unlike {@link CliOption} a
 * {@link CliArgument} is NOT triggered by its {@link #name() name} but is identified by its position. The
 ...
 * {@link #mode() mode} can cause maintenance trouble. This is only supported to avoid limitations (e.g. if
 * you want to implement something like "grep pattern [file]+") but NOT recommended.
 * 
...
public @interface CliArgument {
  ...
  /**
   * The ID of the argument used to identify the argument e.g. via {@link #addCloseTo()}. The default is the
  ...
  /**
   * Determines if this {@link CliArgument} should be add after ( <code>true</code>) or before (
   * <code>false</code>) the {@link CliArgument argument} identified by {@link #addCloseTo()}.
   */
  boolean addAfter() default true;

  /**
   * The {@link #name() name} of the argument where to add this {@link CliArgument} in the list of arguments.
   ...
   * {@link net.sf.mmm.util.nls.api.NlsMessage#getArgument(String) NLS-arguments} are "default" for the
   * default value.
   */
  String usage();
