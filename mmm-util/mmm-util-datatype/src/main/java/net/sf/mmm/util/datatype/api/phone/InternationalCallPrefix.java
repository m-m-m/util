 ...
 * international call prefix (also called exit code or IDD code) for a phone
 * number. It is the sequence that has to be dialed before the
 * {@link CountryCode} to make an international call. The
 * {@link InternationalCallPrefix} is specific for the county the caller is
 * dialing from. Therefore it is typically indicated by the plus sign (+) in
 * internationalized phone numbers. The {@link InternationalCallPrefix} is
 * typically "00" as suggested by the international telecommunications union
 * (ITU). However, some countries have other codes like "011" (NANPA). In
 * Finland there are multiple international call prefixes for different phone
 * carriers. The longest prefix is (so far) 5 digits.
 * 
...
public final class InternationalCallPrefix extends AbstractSimpleDatatype<String> {
  ...
  /**
   * The {@link InternationalCallPrefix} of the NANPA (North American Numbering
   * Plan).
   */
