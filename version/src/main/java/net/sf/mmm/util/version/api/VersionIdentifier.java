...
public interface VersionIdentifier extends Comparable<VersionIdentifier>, AttributeReadValue<String> {
   ...
   * {@link VersionIdentifier} {@code V2} that identifies a <em>potential</em> next release after {@code V1}.
   * <br>
   * <b>ATTENTION:</b><br>
   ...
   * <b>ATTENTION:</b><br>
   * If the {@link #getPhase() phase} is undefined ({@code null}), {@link DevelopmentPhase#RELEASE} will be
   * assumed for {@link #compareTo(VersionIdentifier)}.
   *
   ...
   * {@link #getPhase()} is {@link DevelopmentPhase#RELEASE} this method should return {@code null} (there is
   * just one official release - after that there are {@link DevelopmentPhase#UPDATE updates}). <br>
   * <b>ATTENTION:</b><br>
   ...
   *
   * @return {@code true} if this {@link VersionIdentifier} represents a snapshot release, {@code false} in
   *         case of an official release.
   */
  boolean isSnapshot();
  ...
  /**
   * This method determines a (non-linear) distance from this version to the given {@code otherVersion}. It
   * will return
   * <ul>
   * <li>{@link #COMPARE_TO_INCOMPARABLE} if both versions are incompatible to each other (e.g. ).</li>
   * <li>{@code 0} if both versions are historically equivalent (but may NOT be {@link Object#equals(Object)
   * equal})</li>
