 ...
 * {@link DataResource} as a {@link java.io.File file} but it may come from other sources than the filesystem.
 * <br>
 * The major reason for naming it {@link DataResource} is that {@code Resource} is a very general name already
 * occupied by {@link javax.annotation.Resource}.
 *
...
public interface DataResource {
   ...
   * {@code false}. However in case of {@code true} only {@link #isAvailable()} can guarantee if a resource
   * really exists and contains data. E.g. if the resource points to a {@link java.io.File} then this method
   * can check if it is a {@link java.io.File#isFile() data-file}. So in case it points to a directory or does
   * not exist at all in the filesystem, this method will return {@code false}. Please also note that this may
   * invoke expensive operations if the according directory path points to something like a network share. You
   * should also be aware that the state of {@link #isData()} and {@link #isAvailable()} can change at any
   * time so you never have a full guarantee if some data exists or NOT. However in most cases it is very
   * improbable that this status changes when you {@link #openStream() read} the resource immediately after
   * the check.
   *
   ...
   * @throws ResourceNotAvailableException if this resource is NOT {@link #isAvailable() available}.
   */
  InputStream openStream() throws ResourceNotAvailableException;
  ...
  /**
   * This method retrieves a {@link DataResource} pointing to the given {@code resourcePath} based on this
   * resource.<br>
   ...
   * to be a {@link BrowsableResource#cd(String) cd} command if invoked on a
   * {@link BrowsableResource#isFolder() folder} what is not exactly not the case (instead it is a
   ...
   *
   * @param resourcePath the absolute or relative path pointing to a new resource. If it is a relative path,
   *        it is interpreted relative to the parent URI (directory) of this resource.
   * @return the resource pointing to the given path (relative to this resource).
   * @throws ResourceUriUndefinedException if the given {@code resourcePath} leads to an undefined or illegal
   *         URI.
   */
   ...
   * @return {@code true} if the resource has been modified after the given {@code date}, {@code false} if it
   *         has NOT been modified after the given {@code date} and {@code null} if this can NOT be determined
   *         (resource not {@link #isAvailable() available} or operation NOT supported by this resource).
   * @since 2.0.0
   */
