...
public class ClasspathScannerTest extends Assertions {
  ...
  private static final String CLASSPATH_LOGBACK_XML = "logback.xml";

  private static final String CLASSPATH_CLASSPATHRESOURCE_TXT = "net/sf/mmm/util/resource/base/ClasspathResource.txt";
  ...
  protected final ClasspathScanner getClasspathScanner() {
    ...
    return AbstractClasspathScanner.getInstance();
  }
  ...
  public void testGetByName() {
    ...
    checkFile(CLASSPATH_CLASSPATHRESOURCE_TXT);
    checkFile(CLASSPATH_LOGBACK_XML);
    ClasspathScanner scanner = getClasspathScanner();
  ...
  public void testClasspathScanner() {
    ...
    // in maven there is a minimal classpath while in eclipse the classpath is poluted with lots of eclipse
    // stuff further the number of files may change and hard coding exact numbers will make the test very
    // fragile.
    assertThat(fileList.size()).isBetween(Integer.valueOf(10000), Integer.valueOf(30000));
    assertThat(fileList).contains(scanner.getClasspathResource(CLASSPATH_CLASSPATHRESOURCE_TXT), scanner.getClasspathResource(CLASSPATH_LOGBACK_XML));

    List<String> childern = new ArrayList<>();
    for (BrowsableResource child : scanner.getClasspathResource().getChildResources()) {
      childern.add(child.getPath());
    }
    assertThat(childern).contains("ch", "com", "net", "javax", "org", "META-INF", CLASSPATH_LOGBACK_XML);
  }
  ...
  public void testGetQualifiedName() {
    ...
    BrowsableResource fileResource = scanner.getClasspathResource(CLASSPATH_CLASSPATHRESOURCE_TXT);
    // then
