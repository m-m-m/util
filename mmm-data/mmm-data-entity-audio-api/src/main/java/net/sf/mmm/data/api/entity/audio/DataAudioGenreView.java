 ...
 * genre of an {@link DataAudioView audio track}.
 * 
...
public interface DataAudioGenreView extends DataSelectionTreeView<DataAudioGenreView> {
  ...
  /**
   * This method gets the ID of the best matching ID3 genre. The official
   * standard ID3v1 introduced the genre IDs from <code>0</code> to
   * <code>79</code>. The genre IDs from <code>80</code> to <code>147</code>
   * have been added by Winamp and became more or less defacto standard.
   * However, these genre lists are incomplete and also contain entries that are
   * mixtures of genres with other attributes (like "Christian Gangsta Rap").<br/>
   * In mmm the {@link DataAudioGenreView genres} are organized as tree that
   * allows to add fine grained custom genres derived from official genres.
   * Further, other aspects (like if an {@link DataAudioView audio track} is
   * instrumental or religious) can be expressed by additional attributes in
   ...
   * </table>
   * 
   * @return the best matching ID3 genre ID or <code>null</code> if no such
   *         genre ID can be associated (e.g. for the genre root node).
   */
  Integer getId3Genre();
