 ...
 * statically typed custom java classes that can be extended at runtime with dynamic classes that use the same
 * implementation based on generic and virtual {@link net.sf.mmm.data.api.reflection.DataField fields}.
 * 
...
@SecondaryTable(name = DataEntity.CLASS_TITLE)
public abstract class AbstractDataEntity extends AbstractDataObject implements DataEntity {
   ...
   * <b>ATTENTION:</b><br/>
   * This is an internal method that should never be used by end users. It is used for dynamic classes that
   * are defined at runtime without having an explicit java class as implementation.
   * 
  ...
  /**
   * This method gets the static {@link #getDataClassId() class ID}. It has to be implemented by all
   * non-abstract {@link DataEntity} implementations by simply returning the <code>CLASS_ID</code> constant
   * that is also used for {@link net.sf.mmm.data.api.reflection.DataClassAnnotation#id()}.
   * 
   ...
   * {@link net.sf.mmm.data.api.reflection.DataField property}. If it does not yet exists (no links have been
   * persisted for this property) it will be created.<br/>
   * A {@link DataEntity} may have multiple {@link net.sf.mmm.data.api.link.LinkList}
   * {@link net.sf.mmm.data.api.reflection.DataField fields}. The <code>fieldId</code> should be defined as a
   * constant in a central location (see {@link net.sf.mmm.data.api.reflection.DataFieldIds}). An according
   * getter needs to be implemented that simply delegates to this method using the ID constant. This getter
   * also needs to be annotated with {@link net.sf.mmm.data.api.reflection.DataFieldAnnotation} assigning the
   * ID to {@link net.sf.mmm.data.api.reflection.DataFieldAnnotation#id()}. The getter for that field should
   * look as following:
   * 
   * <pre>
   ...
   * @param fieldId is the {@link net.sf.mmm.data.api.reflection.DataField#getId() ID} of the
   *        {@link net.sf.mmm.data.api.reflection.DataField} reflecting the
   *        {@link net.sf.mmm.data.api.link.LinkList} field.
   * @param entityClass is the class reflecting the {@link AbstractDataEntity entity} the {@link Link}s in
   *        this {@link net.sf.mmm.data.api.link.LinkList} point to.
   * @return the requested {@link MutableLinkList}.
   */
  @SuppressWarnings({ "rawtypes", "unchecked" })
  protected <ENTITY extends DataEntity> MutableLinkList<ENTITY> getLinklist(long fieldId, Class<ENTITY> entityClass) {
  ...
  /**
   * This method initializes the property {@link #fieldId2LinklistMap} based on {@link #getAllLinkList()}.
   */
  ...
  private synchronized void initializeFieldId2LinklistMap() {
    ...
    if (this.fieldId2LinklistMap == null) {
      ...
      for (LinkImpl<? extends AbstractDataEntity> link : getAllLinkList()) {
        ...
        MutableLinkListImpl<? extends AbstractDataEntity> linkList = this.fieldId2LinklistMap.get(fieldId);
        if (linkList == null) {
          linkList = new MutableLinkListImpl<AbstractDataEntity>(this, fieldId.longValue(), this.allLinkList);
