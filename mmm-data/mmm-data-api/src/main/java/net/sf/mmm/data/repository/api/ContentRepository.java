...
import net.sf.mmm.data.repository.api.access.ContentObjectReadAccess;
...
public interface ContentRepository extends ContentObjectReadAccess, ContentObjectWriteAccess {
   ...
   * @see net.sf.mmm.data.api.ContentObject#isFolder()
   * 
   ...
   * <code>{@link #get(ContentId)}</code>) will be "manipulated" to add support
   ...
   *        repository} (e.g. via <code>{@link #get(ContentId)}</code>).
   ...
   * @param newName is the new {@link ContentObject#getTitle() name} of the
   *        content-object.
   * @throws ContentException if the operation failed. This can have one of the
   *         following reasons:
   *         <ul>
   ...
   *         {@link net.sf.mmm.data.reflection.api.ContentClass content-class} and
   ...
   *         {@link ContentObject#getTitle() named} <code>newName</code>.</li>
   *         </ul>
   */
