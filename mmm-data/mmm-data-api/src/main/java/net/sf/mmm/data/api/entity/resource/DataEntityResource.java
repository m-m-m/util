 ...
 * {@link net.sf.mmm.data.api.entity.DataEntity} that represents a node in a repository resource tree (like a
 * filesystem). <br/>
 * A {@link DataEntityResource resource} is either a {@link DataFolder folder} or a {@link DataFile file}.<br/>
 * 
...
public interface DataEntityResource extends DataEntity, DataNode<DataFolder> {
   ...
   * <b>ATTENTION:</b><br/>
   * The title of a resource has to unique for all resources with the same {@link #getParent() parent}. In
   * other words it is prohibited that a {@link DataFolder} has two {@link DataFolder#getChildren() children}
   * with the same {@link #getTitle() title}.<br/>
   * 
   * {@inheritDoc}
   */
  @Override
