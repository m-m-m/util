...
public interface MutableLinkList<CLASS extends DataEntity> extends LinkList<CLASS> {
   ...
   * @param classifier is the {@link Link#getClassifier() classifier} of the link.
   ...
   * 
   * @param index is the position of the existing link to replace. Has to be in the range from <code>0</code>
   *        to <code>{@link #getLinkCount()}-1</code>.
   * @param targetEntity is the {@link Link#getTarget() target} of the link.
   * @param classifier is the {@link Link#getClassifier() classifier} of the link.
   * @return the link that has been replaced and was located at the given index before this operation.
   */
