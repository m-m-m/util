...
public class DescriptorBuilder {
   ...
   * The constructor.
   * 
   * @param loader is the classloader used to load classes.
   */
   ...
   * {@link ExtendedComponentDescriptor#getConstructor() constructor} used to
   * instantiate the
   ...
   * 
   * @param <I> is the templated type of the
   *        {@link ExtendedComponentDescriptor#getImplementation() component implementation}.
   * @param implementation is the
   *        {@link ExtendedComponentDescriptor#getImplementation() component implementation}.
   * @return the requested constructor.
   * @throws ComponentException if the given <code>implementation</code> is
   *         illegal (e.g. has no public constructor).
   */
  ...
  public String getInstantiationPolicy(Class<?> implementation) {
    ...
    Resource classAnnotation = AnnotationUtil.getClassAnnotation(implementation, Resource.class);
  ...
  /**
   * This method creates the {@link ConstructorDependency} values for the given
   * <code>constructor</code> and adds them to the given
   * <code>dependencies</code>.
   * 
   * @param constructor is the constructor to build the dependencies for.
   * @param dependencies is the list where to add the dependencies.
   */
  public void addConstructorDependencies(Constructor<?> constructor, List<Dependency> dependencies) {
    ...
    if (constructorArgs.length > 0) {
      ...
      for (int i = 0; i < constructorArgs.length; i++) {
         ...
         * constrcutorAnnoations[i][j].getClass()) { constructorArgResource =
         * (Resource) constrcutorAnnoations[i][j]; break; } }
         */
  ...
  public <S, I extends S> ExtendedComponentDescriptor<S, I> createDescriptor(
    ...
    for (int i = 0; i < methods.length; i++) {
      ...
      if (parameterTypes.length == 1) {
        Resource resourceAnnotation = AnnotationUtil
            .getMethodAnnotation(methods[i], Resource.class);
        if (resourceAnnotation != null) {
          String instanceId = resourceAnnotation.name();
          AbstractDependency<?> dependency = new SetterDependency(methods[i], parameterTypes[0],
              instanceId);
      ...
      if (parameterTypes.length == 0) {
        // lifecycle methods???
        PostConstruct init = AnnotationUtil.getMethodAnnotation(methods[i], PostConstruct.class);
        if (init != null) {
          lifecycleMethods
              .add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_INITIALIZE, methods[i]));
        }
        PreDestroy dispose = AnnotationUtil.getMethodAnnotation(methods[i], PreDestroy.class);
        if (dispose != null) {
          lifecycleMethods.add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_DISPOSE, methods[i]));
        }
        String methodName = methods[i].getName();
        if (LifecycleMethod.LIFECYCLE_START.equals(methodName)) {
          lifecycleMethods.add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_START, methods[i]));
        } else if (LifecycleMethod.LIFECYCLE_STOP.equals(methodName)) {
          lifecycleMethods.add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_STOP, methods[i]));
        } else if (LifecycleMethod.LIFECYCLE_PAUSE.equals(methodName)) {
          lifecycleMethods.add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_PAUSE, methods[i]));
        } else if (LifecycleMethod.LIFECYCLE_RESUME.equals(methodName)) {
          lifecycleMethods.add(new LifecycleMethod(LifecycleMethod.LIFECYCLE_RESUME, methods[i]));
        }
  ...
  /**
   * 
   * @param specificationName is the {@link Class#getName() name} of the
   *        component
   *        {@link ExtendedComponentDescriptor#getSpecification() specification}
   * @param implementationName is the {@link Class#getName() name} of the
   *        component
   *        {@link ExtendedComponentDescriptor#getImplementation() implementation}
   * @return the according descriptor.
   */
  ...
  public static void main(String[] args) {
    ...
    System.out
        .println(AnnotationUtil.getClassAnnotation(DescriptorBuilder.class, PreDestroy.class));
  }
