...
public final class FieldDependency<S> extends AbstractDependency<S> {
   ...
   * The constructor.
   * 
   * @param injectionField is the {@link #getInjectionField() field} where to
  ...
  public FieldDependency(Field injectionField, Class<S> specification, String instanceId) {
    ...
    if (!this.field.getType().isAssignableFrom(specification)) {
      throw new IllegalArgumentException("Field '" + injectionField.getName() + "' has type '"
          + injectionField.getType() + "' which is NOT assignable to '" + specification + "'!");
    }
  }
