 ...
 * {@link net.sf.mmm.ui.toolkit.api.view.UiNode}.<br/>
 ...
 * {@link net.sf.mmm.ui.toolkit.api.view.UiNode}-hierarchy and inherit different implementations (Swing, SWT,
 * GWT, etc.) from that without creating redundant code.
 * 
...
public interface UiNodeAdapter<DELEGATE> extends AttributeWriteHtmlId, AttributeWriteStylesAdvanced, AttributeWriteVisible,
  ...
  /**
   * This method gets the top-level UI object of this node. This is typically the same as the
   * {@link #getDelegate() delegate}. However more complex widgets may be composed out of multiple native UI
   * objects. In such case this method can be overridden to return the top-level delegate used for composition
   * e.g. via
   * {@link net.sf.mmm.ui.toolkit.api.view.composite.UiExtendableComposite#addChild(net.sf.mmm.ui.toolkit.api.view.UiElement)}
   * as well as to update {@link AttributeWriteVisible#setVisible(boolean) visibility}. However
   * {@link #getDelegate() delegate} still returns the active part of the composed widget used to update
   ...
   * 
   * @param newParent is the new parent node. May be <code>null</code> to remove this object from the UI tree.
   */
