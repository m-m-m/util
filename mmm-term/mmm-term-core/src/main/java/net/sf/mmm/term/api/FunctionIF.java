/* $Id: FunctionIF.java 191 2006-07-24 21:00:49Z hohwille $ */
 ...
 * returned by {@link FunctionIF#getName()}. And another string constant named
 * <code>SYMBOL</code> should contain the
 * {@link FunctionIF#getOperatorSymbol() "operator symbol"}.
 * 
...
public interface FunctionIF {
     ...
     * short english term describing the meaning of the function. The name can
     * be used in an expression string to call the function. E.g. if a function
     * is named <code>add</code> the expression <code>add(1, 2)</code> would
     ...
     * if the function can not be used as operator. Please consider to define
     * the proper {@link FunctionIF#getOperatorPriority() priority} if you
     * enable the operator usage.<br>
     * The operator can be used for unary function usage (e.g. "!true") or
     * multi-argument usage (e.g. "3+5+4"). Non-arg usage is NOT possible via
     * the operator.
     * 
     * @return the unique operator symbol or <code>null</code> if the function
     *         is no operator. The operator symbol MUST NOT be "" and consist
     *         only legal charactes as described by
     *         {@link #LEGAL_OPERATOR_CHARS}. Further the operator symbol
     *         should explain the meaning of the function as much as possible.
     */
    String getOperatorSymbol();

    /**
     * This method get the number of minimum number of arguments required by
     * this function for calculation. The value must be greater or equal than
     * zero and less or equal than the maximum argument count returned by the
     * method {@link FunctionIF#getMaximumArgumentCount()}.
     * 
    ...
    /**
     * This method gets the maximum number of arguments required by this
     * function for calculation. The value must be greater or equal than the
     * minimum argument count returned by the method
     * {@link FunctionIF#getMinimumArgumentCount()}. If the number of arguments
     * is not limited, return <code>Integer.MAX_VALUE</code>.
     * 
    ...
    /**
     * This method validates that the given number of arguments is legal for
     * this function.
     * 
     * @param count
     ...
     *         {@link FunctionIF#getMinimumArgumentCount()} or greater than
     *         {@link FunctionIF#getMaximumArgumentCount()} or any number within
     ...
     * {@link #getOperatorSymbol() opeator}. E.g. let there be the two
     * operators <b>+</b> and <b>*</b>. Further an expression is given by the
     * string <code>1+2*3</code>. Now if the priority of <b>*</b> is higher
     * than the priority of <b>+</b> the expression will be parsed as
     * <code>1+(2*3)</code>. Else (if the priority of <b>*</b> is less or
     * equal to <b>+</b>) the expression will be parsed as <code>(1+2)*3</code>.<br>
     ...
     * {@link FunctionIF#getOperatorSymbol() "operator symbol"} is
     * <code>null</code>.
     * 
