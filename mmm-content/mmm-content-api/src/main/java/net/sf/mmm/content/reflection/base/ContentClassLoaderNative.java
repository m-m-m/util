 ...
 * load {@link ContentClass}es from the native Java implementations of entities.
 * 
...
public class ContentClassLoaderNative extends AbstractContentClassLoader {
  ...
  public AbstractContentClass loadClassRecursive(Class<?> javaClass, Context context) {
    ...
    ContentClassAnnotation contentClassAnnotation = javaClass
    ...
    if (contentClass == null) {
      ...
      ContentClassModifiers contentClassModifiers = ContentClassModifiersBean.getInstance(isSystem,
    ...
    if (createSuperClass) {
      ...
      if (!this.rootEntity.equals(javaClass)) {
        ...
        while (!superClass.isAnnotationPresent(ContentClassAnnotation.class)) {
  ...
  protected void loadFieldsRecursive(AbstractContentClass contentClass) {
    ...
    for (PojoPropertyDescriptor methodPropertyDescriptor : methodPojoDescriptor
      ...
      try {
        ...
        ContentFieldAnnotation contentFieldAnnotation = null;
        if (accessor == null) {
          // no getter: ignore
          declareField = false;
        } else {
          Method getter = (Method) accessor.getAccessibleObject();
          contentFieldAnnotation = getter.getAnnotation(ContentFieldAnnotation.class);
        ...
        if (declareField) {
          ...
          ContentFieldModifiers contentFieldModifiers = ContentFieldModifiersBean.getInstance(
  ...
  protected ContentFieldAccessor getFieldAccessor(PojoPropertyDescriptor methodPropertyDescriptor) {
    ...
    return new ContentFieldAccessorPojo(getter, setter);
  }
