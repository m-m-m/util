...
public abstract class EntityTo<ID> extends DataTo implements MutableRevisionedEntity<ID> {
   ...
   * This is set by bean-mapping. In such case the modification counter is updated from that reference by this
   * method as an intended side effect. This is done because JPA implementations will typically update the
   * modification counter only after the transaction has been committed and the conversion of an entity to
   * {@link EntityTo ETO} typically happens before. This approach is better and more robust than
