 ...
 * <em>entity</em> or <em>business-object</em>. It is supposed to be a simple java-bean or in words a
 ...
 * {@link net.sf.mmm.util.transferobject.api.TransferObject}s. Our recommendation is to
 * {@link net.sf.mmm.util.transferobject.api.TransferObjectUtil#convertFromEntity(PersistenceEntity, Class) convert}
 * persistent entities into {@link net.sf.mmm.util.transferobject.api.EntityTo entity transfer-objects} when exposing
 * functionality in the API of the <em>logic</em> layer.
 *
...
public interface GenericEntity<ID> extends AttributeReadId<ID>, Entity {
  ...
  /**
   * This method gets the unique identifier (primary key) of this entity. While this method is initially defined in a
   * generic way, it is strongly recommended to use {@link Long} as datatype for IDs. <br>
   * Even if you want to have a {@link String}-based business-oriented identifier it is best practice to use a
   * {@link Long} as primary key and add the business identifier as additional field (with a unique constraint). This
   * way references to the entity will be a lot more compact and improve your performance in JOINs or the like. However,
   * there may be reasons to use other {@link net.sf.mmm.util.lang.api.Datatype datatypes} for the ID. In any case the
   * unique ID should be an immutable java-object that can be rebuild from its {@link Object#toString()
   * string-representation}. <br>
   * Please note that if your ID has a specific syntax, semantic, formatting rules, etc. you should create a custom
   * {@link net.sf.mmm.util.lang.api.Datatype} for it. If it can easily be mapped to a {@link Long} value you can still
   * use {@link Long} here and provide a transient getter method that returns the your custom
   * {@link net.sf.mmm.util.lang.api.Datatype} from the {@link Long}.
   *
   * @return the ID of this entity. It may be {@code null} if the object is {@link PersistenceEntity#STATE_NEW new} and
   *         no ID has manually been assigned.
   */
  @Override
  ID getId();
   ...
   * {@link javax.persistence.EntityManager#persist(Object) saved}, this counter will be increased. The initial value
   * after construction is {@code 0}. <br>
