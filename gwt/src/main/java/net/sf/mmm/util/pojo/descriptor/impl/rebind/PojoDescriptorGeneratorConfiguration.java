...
/**
 * This is the configuration for {@link PojoDescriptorGenerator} and {@link PojoDescriptorBuilderGenerator}. If you want
...
public interface PojoDescriptorGeneratorConfiguration {
   ...
   * <b>ATTENTION:</b><br>
   * You need to require some base class or marker-interface for your {@link net.sf.mmm.util.pojo.api.Pojo}s. Otherwise
   * there is way too much clutter generated and your web-application will grow too big. So never consider to return
   * {@code true} for every given type.
   *
   * @param type is the {@link net.sf.mmm.util.pojo.api.Pojo} to check.
   * @param typeOracle is the {@link TypeOracle} that may be used to {@link TypeOracle#findType(String) resolve}
   *        additional types (e.g. a marker interface).
   ...
   *         {@link JClassType#getSuperclass()}) that is still {@link #isPojoTypeSupported(JClassType, TypeOracle)
   *         supported} or {@code null} if no such type exists.
   */
  JClassType getSupportedSuperType(JClassType type, TypeOracle typeOracle);

  /**
   * @return a description of the criteria for the {@link #isPojoTypeSupported(JClassType, TypeOracle) supported}
   *         {@link net.sf.mmm.util.pojo.api.Pojo}s. Only used for debugging.
   */
