...
public final class ConfigurationUtil {
  ...
  /**
   * This method initializes the given {@link MutableContextIF environment} with
  ...
  public static void initializeContext(MutableContextIF context) {
    ...
    key = ConfigurationFactoryIF.CONTEXT_VARIABLE_PREFIX + PropertiesFactory.CONTEXT_DEFAULT_NAME
        + PropertiesFactory.CONTEXT_VARIABLE_SUFFIX_FLAT;
   ...
   */
  public static ConfigurationAccessFactoryIF getAccessFactory(ConfigurationIF includeConfiguration,
      ContextIF env) {

    String access = includeConfiguration.getDescendant(ConfigurationDocumentIF.NAME_INCLUDE_ACCESS)
        .getValue().getString(null);
    if (access == null) {
      access = env.getValue(ConfigurationAccessFactoryIF.CONTEXT_VARIABLE_DEFAULT).getString();
    }
    String varPrefix = ConfigurationAccessFactoryIF.CONTEXT_VARIABLE_PREFIX + access;
    String varFactory = varPrefix + ConfigurationAccessFactoryIF.CONTEXT_VARIABLE_SUFFIX_FACTORY;
