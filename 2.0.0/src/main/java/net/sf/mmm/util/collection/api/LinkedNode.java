 ...
 * {@link net.sf.mmm.util.collection.base.BasicLinkedNode} instead.
 * 
 * 
...
public interface LinkedNode<V, NODE extends LinkedNode<V, NODE>> {
   ...
   * 
   * @return the next {@link LinkedNode node} or <code>null</code> if this is
   *         the last node (tail).
   */
  NODE getNext();
  ...
  /**
   * This method determines if this node is linked. A node is linked if it has a
   * {@link #getNext() next node}.
   * 
  ...
  /**
   * This method appends the {@link #getValue() values} of the list represented
   * by this node to the given <code>list</code>.<br/>
   * Call this method on the head-node with an empty {@link List} to convert a
   * {@link LinkedNode}-list to a regular {@link List}.
   * 
   ...
   * {@link List}.
   * 
   * @return the {@link List} of all {@link #getValue() values} of the list
