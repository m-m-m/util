...
public interface Message extends Serializable, AttributeReadUuid, AttributeReadMessage, AttributeReadMessageCode {
   ...
   * <b>ATTENTION:</b><br>
   * This method is designed for server applications with <em>native language support</em> (NLS). On client
   * side (e.g. for GWT clients) only a single locale may be supported at a time and this method may behave
   * like {@link #getMessage()} ignoring the {@link Locale}.
   *
   ...
   * <ul>
   * <li>{@value #TYPE_VALIDATION_FAILURE} - for a {@code ValidationFailure}.</li>
   * <li>{@value #TYPE_USER_ERROR} - for an {@link Throwable error} suitable for the end-user (e.g. caused by
   * providing invalid data). Unlike {@link #TYPE_VALIDATION_FAILURE} this is an actual error that has been
   * detected after a successful validation.</li>
   * <li>{@value #TYPE_TECHNICAL_ERROR} - for an {@link Throwable error} related to a technical problem (e.g.
   * programming failure such as {@link NullPointerException} or operational error like
   ...
   *         type} is {@link #TYPE_TECHNICAL_ERROR} or {@link #TYPE_USER_ERROR}. Will be {@code null} if not
   *         available.
   */
  @Override
  UUID getUuid();
