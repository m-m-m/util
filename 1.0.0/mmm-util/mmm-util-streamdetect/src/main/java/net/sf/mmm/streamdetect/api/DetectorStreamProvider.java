 ...
 * {@link DetectorStream detector-streams}. <br>
 ...
 * 
 *   {@link Context} metadata = detectorOutStream.{@link DetectorStream#getMetadata() getMetadata()}; 
 *   Long bytesWritten = metadata.{@link Context#getValue(String) getValue}("file.size").{@link net.sf.mmm.value.api.GenericValue#getLong()};
 * </pre>
 * 
...
public interface DetectorStreamProvider {
   ...
   * values, that are mutable, the stream wrapper manipulates the data such that
   * the given values correspond to the data.<br>
   * E.g. if the metadata contains a title and a genre and the stream points to
   * the data of an mp3 song, the given title and genre are "written" to the ID3
