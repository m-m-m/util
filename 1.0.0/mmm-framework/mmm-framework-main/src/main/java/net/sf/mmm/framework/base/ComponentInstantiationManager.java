...
public interface ComponentInstantiationManager<S, I extends S> {
  ...
  /**
   * This method is called when a component instance is released. It decides if
   * the component in the given <code>instanceContainer</code> will be
   * shutdown or kept untouched to be reused.
   * 
   * @param instanceContainer is the container with the released component
   *        instance. This container should have been retrieved from the method
   *        {@link #request(String)}.
   ...
   * instantiation manager. The instantiation manager must return all containers
   * with instances that are shared or pooled (where
