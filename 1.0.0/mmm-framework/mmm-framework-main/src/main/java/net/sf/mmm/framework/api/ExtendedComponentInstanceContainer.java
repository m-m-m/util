...
/**
 * This is the interface for a {@link ComponentInstanceContainer container} that
 * also contains internal details of a managed component.
 * 
 * @param <S> is the
...
public interface ExtendedComponentInstanceContainer<S, I extends S> extends
   ...
   * that has been performed. In some situations the component might change its
   * state independently. E.g. if the phase
   * {@link net.sf.mmm.framework.api.LifecycleMethod#LIFECYCLE_EXECUTE "execute"}
   * was be performed and has completed, the {@link #getLifecycleState() state}
   * will become
   * {@link net.sf.mmm.framework.api.LifecycleMethod#LIFECYCLE_EXECUTED "executed"}.
   * 
   * @return the current lifecycle state or <code>null</code> if the lifecycle
   ...
   * 
   * @param state is the new lifecycle phase to set.
   */
