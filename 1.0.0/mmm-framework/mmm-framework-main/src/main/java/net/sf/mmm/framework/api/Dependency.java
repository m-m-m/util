...
public interface Dependency<S> {
  ...
  /**
   * This method gets the
   * {@link ComponentManager#requestComponent(Class, String) instance-ID} of the
   * dependency.
   * 
   ...
   * instance of the dependency {@link #getSpecification() specification}. The
   * method must take a {@link Method#getParameterTypes() single argument} that
   * is {@link Class#isAssignableFrom(Class) assignable} from the
   ...
   * instance of the dependency {@link #getSpecification() specification}. The
   * field {@link Field#getType() type} must be
   ...
   * this prevents easy testing. This injection type is only supported to allow
   * integrating existing components using field injection.
   * 
  ...
  /**
   * An enumeration of the availabe {@link #getInjectionType() injection-types}.
   */
