 ...
 * <ul>
 * <li>A component sending events only needs to know the {@link EventBus} but not the receivers of the
 * events.</li>
 * <li>A component receiving events only needs to know the {@link EventBus} but not the sender of the
 * events.</li>
 * </ul>
 ...
 * This interface exists only for the high abstraction needed for {@code mmm-client}. If you want to write a
 * portable client application that can run in different environments (GWT, JavaFx, etc.) then you should use
 * this simplified but normalized API. Otherwise you may want to use an external event bus implementation
 * directly such as <a href="https://code.google.com/p/guava-libraries/wiki/EventBusExplained">guava event
 * bus</a>. <br>
 * <b>NOTE:</b><br>
