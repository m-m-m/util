...
/**
 * A {@link CliMode} is used to annotate a {@link CliClass CLI annotated class} in order to define a operational mode.
 * Such {@link CliMode mode} is {@link CliOption#mode() referred} by a {@link CliOption} or {@link CliArgument} in order
 * to group them. This allows to express which {@link CliOption options} and {@link CliArgument arguments} can be mixed
 * and which of them should NOT go together. <br>
 * E.g. the options "--help" does NOT make sense to be mixed with "--shutdown". Within some mode, {@link CliOption
 * options} may be {@link CliOption#required() required} but in another mode they should NOT be present. <br>
 ...
 * {@link CliMode#isAbstract() abstract} mode can only be used together with another one that {@link CliOption#mode()
 * has} a mode that is NOT {@link CliMode#isAbstract() abstract} and {@link #parentIds() extends} the
