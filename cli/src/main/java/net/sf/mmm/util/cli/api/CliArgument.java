...
/**
 * A {@link CliArgument} is used to annotate a property (member variable of some class or setter-method) that should be
 * set from a main-program via a commandline argument. Unlike {@link CliOption} a {@link CliArgument} is NOT triggered
 * by its {@link #name() name} but is identified by its position. The first commandline parameter that is no
 * {@link CliOption option} or occurs after the argument "--" is treated as the first argument and further parameters
 * can also only be {@link CliArgument arguments}. Additionally the last {@link CliArgument} -property can have the type
 * {@link java.util.List} if a proper generic is used to declare the item-type. <br>
 * E.g. a program {@code Foo} may have one {@link CliArgument argument} of type {@link java.util.List}{@literal <}
 * {@link java.io.File}{@literal >} and you invoke {@code Foo file1 file2 file3} then the property is set to a
 * {@link java.util.List} equivalent to {{@code new File("file1"), new File("file2"), new File("file3")}}. <br>
 * <b>ATTENTION:</b><br>
