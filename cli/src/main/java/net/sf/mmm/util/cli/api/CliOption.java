...
/**
 * A {@link CliOption} is used to annotate a property (member variable of some class or setter-method) that should be
 * set from a main-program via a commandline option. <br>
 * If the annotated property is a {@link java.lang.reflect.Field} it may NOT be {@link java.lang.reflect.Modifier#STATIC
 * static} or {@link java.lang.reflect.Modifier#FINAL final} and by convention it should be
 * {@link java.lang.reflect.Modifier#PRIVATE private}. Setters may NOT be {@link java.lang.reflect.Modifier#STATIC
 * static} and by convention they should be {@link java.lang.reflect.Modifier#PUBLIC public}. <br>
 * The annotated property should be initialized properly at construction in order to determine whether the option was
 * triggered or not. For non-primitive types the property should be initialized with {@code null}. <br>
 * When the main-program is invoked an {@link CliOption} can be triggered via its {@link #name() name} or one of its
 * {@link #aliases() aliases}. Options with a type other than {@code boolean} need to be followed by a value as CLI
 * parameter. This value has to be quoted in the commandline if it contains whitespaces or other characters that are
 * interpreted by the shell (e.g. backslash in Unix). <br>
 * The type of the annotated property has to be supported by the CLI implementation so the value can be converted
 * properly. The following types are guaranteed to be supported:
 * <table border="1">
 * <tr>
 * <th>Type</th>
 * <th>Comment</th>
 ...
 * <tr>
 * <td>boolean</td>
 * <td>Set to {@code true} if {@link CliOption} is present. Should be initialized with {@code false} at construction.
 * </td>
 * </tr>
 * <tr>
 * <td>{@link Boolean}</td>
 * <td>If the option is present with a value of {@code true} or {@code false} the according {@link Boolean} is applied.
 * Otherwise values are treated as invalid. According to {@link CliStyle}, {@code true} may be omitted.</td>
 * </tr>
 * <tr>
 * <td>{@link Number}</td>
 ...
 * <tr>
 * <td>{@link Enum}</td>
 * <td>A {@link Class#isEnum() concrete subclass} of {@link Enum}. The value needs to match the {@link Enum#name()} but
 * case is ignored and underscores may also be replaced by hyphen ("-") or whitespace (" ").</td>
 * </tr>
 * </table>
