...
public interface ComposedByteBuffer extends ByteBuffer {
   ...
   * method allows efficient processing of bytes from {@code byte[]} rather then calling
   ...
   *        {@code 0} to <code>{@link #getByteArrayCount()} - 1</code>. A value of {@code 0}
  ...
  /**
   * This method fills the given {@code buffer} starting at {@code offset} with the {@link #next()
   * next} bytes from this {@link ComposedByteBuffer}. The bytes that are filled into the given
   * {@code buffer} will therefore be consumed. This method can be used to write data from this buffer to
   ...
   * @param length is the expected number of bytes to fill into {@code buffer}. Has to be positive and
   *        less or equal to {@code buffer.length - offset}. However at most the number of
   *        {@link #getBytesAvailable() available bytes} can be filled even if {@code length} is greater.
   * @return the number of bytes that have actually been filled into the {@code buffer}. Will be positive
   *         and less or equal to the given {@code length}. Should be only less than
   *         {@code maxLength} if NOT enough {@link #getBytesAvailable() bytes are available}.
   */
