 ...
 * <b>ATTENTION:</b><br>
 * Unfortunately there is no way in java to give a read-only view on a {@code byte[]} than can be
...
public interface ByteArray extends ByteProvider {
   ...
   * be in the range from {@code 0} to <code>{@link #getMaximumIndex() maximumIndex} + 1</code>. This is
   ...
   * {@code 0} to <code>{@link #getMaximumIndex() maximumIndex} + 1</code>.
   * 
   ...
   * {@code -1} to <code>{@link #getBytes()}.length - 1</code>. <br>
   * A negative value ({@code -1}) indicates that the {@link #getBytes() buffer} does NOT contain data
   * (payload).
   * 
   ...
   * indices. <br>
   * <b>ATTENTION:</b><br>
   * If the implementation is immutable and the given {@code minimum} and {@code maximum} index are
   * both equal to the current indices of this {@link ByteArray} this method may return the instance itself (
   * {@code this}) rather than creating a new one.
   * 
   ...
   *        {@link #getMinimumIndex() minimumIndex} of {@code this} {@link ByteArray}.
