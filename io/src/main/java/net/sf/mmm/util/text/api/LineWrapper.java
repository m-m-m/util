...
/**
 * This is the interface used to {@link #wrap(Appendable, TextTableInfo, String, TextColumnInfo) wrap} texts if they
 * exceed a specific length. It even allows to do complex {@link #wrap(Appendable, TextTableInfo, TextColumn[])
 * multi-column layouts} easily.
 *
...
public interface LineWrapper {
  ...
  /**
   * This method {@link Appendable#append(CharSequence) appends} the given {@code text} to the {@code appendable}. If
   * the text is longer than the {@link TextColumnInfo#getWidth() number of characters} left in the current line, the
   * {@code text} is wrapped. This means the current line is filled with parts of the {@code text}, a newline is
   * {@link Appendable#append(CharSequence) added} and the next line is filled with the same procedure until the entire
   * {@code text} is processed. The last line is NOT terminated with a newline but ends with the last character of the
   * text.
   *
   ...
   * @param columnText is the text to append. The text may contain newline characters to indicate a line-break is forced
   *        at those positions.
   ...
   * @return the number of lines that have been {@link Appendable#append(CharSequence) appended} excluding the first
   *         line. In other words the number of {@link net.sf.mmm.util.lang.api.StringUtil#getLineSeparator() line
  ...
  /**
   * This method is like {@link #wrap(Appendable, TextTableInfo, String, TextColumnInfo)} but for a two-column layout.
   *
  ...
  /**
   * This method is like {@link #wrap(Appendable, TextTableInfo, String, TextColumnInfo)} but for a three-column layout.
   *
   * @see #wrap(Appendable, TextTableInfo, TextColumn[])
   *
   * @param appendable is the target where to {@link Appendable#append(CharSequence) append} to.
