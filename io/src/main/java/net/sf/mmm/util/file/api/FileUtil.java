...
public interface FileUtil extends FileUtilLimited {
   ...
   * @param destination is the file to copy to. It will be created if it does NOT exist and overridden
   *        otherwise.
   * @param keepFlags - {@code true} if the flags of the file should be copied as well,
   *        {@code false} otherwise (a new file is created with default flags and only the content is
   *        copied).
   * @throws RuntimeIoException if the operation fails.
   */
   ...
   *        the returned permissions. It may be {@code null} to apply the permissions to all distinct
   *        classes.
  ...
  /**
   * This method sets the {@link FileAccessPermissions permissions} of the given
   * {@code file}. <br>
   * <b>ATTENTION:</b><br>
  ...
  /**
   * This method copies the file or directory given by {@code source} into the given
   * {@code destination}. <br>
   * <b>ATTENTION:</b><br>
   ...
   *        {@link RuntimeIoException} is thrown, else if {@code true} the {@code destination} will
   *        be overwritten.
   * @throws RuntimeIoException if the operation fails.
   */
   ...
   * {@code directory} recursively. If the given {@code directory} denotes an {@link File#exists()
   * existing} {@link File#isDirectory() directory} then it will be empty after the call of this method, else
  ...
  /**
   * This method gets all {@link File files} matching to the given {@code path} and {@code fileType}
   * . The {@code path} may contain {@link net.sf.mmm.util.pattern.base.PathPatternCompiler wildcards}. <br>
   * Examples:
   * <ul>
   * <li>
   ...
   * {@code cwd} or any of its transitive {@link File#list() sub-folders} that end with ".xml"</li>
   * </ul>
   *
   ...
   *        {@link File#isDirectory() directory}. If the given {@code path} is NOT
   ...
   *        {@code cwd}.
   * @param fileType is the type of the requested files or {@code null} if files of any type are
   *        acceptable.
   * @return an array containing all the {@link File files} that match the given {@code path} and apply
   *         to {@code ignore}
   */
  File[] getMatchingFiles(File cwd, String path, FileType fileType);

  /**
   * This method adds all files matching the given {@code path} and {@code fileType} to the
   * {@code list}. The {@code path} may contain
   ...
   *        absolute} it is interpreted relative to the {@link File#isDirectory() directory} given by
   *        {@code cwd}.
   * @param fileType is the type of the files to collect or {@code null} if files of any type are
   *        acceptable.
   * @param list is the list where to {@link List#add(Object) add} the collected files.
   * @return {@code false} if the path is a regular string and {@code true} if the given path
   *         contains at least one {@link net.sf.mmm.util.pattern.base.GlobPatternCompiler wildcard} (
   *         {@code '*'} or {@code '?'}).
   */
