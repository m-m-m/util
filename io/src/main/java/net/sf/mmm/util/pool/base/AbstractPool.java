...
public abstract class AbstractPool<E> implements Pool<E> {
   ...
   *
   * <b>ATTENTION:</b><br>
   * This constructor uses a default capacity of {@value #DEFAULT_CAPACITY}. A suitable default capacity depends on the
   * type of objects that are pooled ({@literal <E>}).
   */
  public AbstractPool() {
   ...
   * @param threadsafe if {@code true} the pool needs to be accessed by multiple {@link Thread}s concurrently,
   ...
   * @param lock is an explicit synchronization-lock object. It may be {@code null}. If it is NOT {@code null},
   *        {@code threadsafe} has to be {@code true}.
   */
   ...
   * {@link #release(Object) released} and will be stored in the pool. The implementation depends on the type of
   ...
   * trusted enough (because it may be vulnerably).
   *
   * @param element the element to reset.
   * @return {@code true} if the given {@code element} can be reused and should be added to the pool, {@code false}
   *         otherwise.
   */
  ...
  /**
   * This method gets the capacity of this pool. This value is the maximum {@link #getSize() size} of this pool. If that
   * size is reached further {@link #release(Object) released} objects are NOT pooled and can be freed by the
   * garbage-collector. <br>
   * <b>ATTENTION:</b><br>
   * The optimal capacity of a {@link Pool} depends heavily on the type of objects that are pooled. For some lightweight
   * objects a very high capacity may suite. However for big buffers (e.g. {@code char[65536]}) the capacity should be
   * very low because otherwise your heap might get crowded.
   *
  ...
  /**
   * This method clears the complete pool. After the call of this method, the pool will be {@link #isEmpty() empty}.
   */
  public void clear() {
