...
/**
 * This class represents a {@link InputStream} that works like a {@link java.io.BufferedInputStream} but exposes its
 * internal state to be used for lookahead operations. <br>
 * <b>ATTENTION:</b><br>
...
public class BufferInputStream extends InputStream implements ByteProcessable {
   ...
   *
   * @param inStream is the {@link InputStream} to adapt.
   * @param capacity is the capacity for each of the two {@link ByteArrayBufferImpl} s. Please note that therefore the
   *        double amount of memory is allocated.
   */
  ...
  /**
   * This method (re-)fills the internal buffer as far as data is available from the underlying {@link InputStream}
   * without {@link #read() consuming} data from the stream. If the internal buffer is already filled, the call of this
   * method will have no effect. <br>
   *
   * @return {@code true} if the end of the stream was encountered while (re)filling the internal buffer, {@code false}
   *         otherwise.
  ...
  /**
   * This method creates a new {@link ProcessableByteArrayBuffer buffer} for lookahead operations on the data from the
   ...
   * this stream will refill the returned buffer.
   *
  ...
  /**
   * This inner class is the {@link ByteProcessor} used to copy bytes from the buffer to the caller consuming data from
   * this stream.
   */
