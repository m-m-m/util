...
public interface PojoUtilLimited {
  ...
  /**
   * This method creates a copy (e.g. via copy-constructor or {@link Cloneable clone}) of the given {@code value}. If
   * the given {@code value} is immutable (like {@link String}, {@link Long}, etc.) it is legal to return the same given
   * instance.
   *
   ...
   * return {@code true} if the given {@code value} is a {@link String}, {@link Boolean}, {@link Character}, anything
   * derived from {@link Number}, an {@link Enum}, an instance of {@link net.sf.mmm.util.lang.api.Datatype}, a
   ...
   * @return {@code true} if the given value is a (immutable) {@link net.sf.mmm.util.lang.api.Datatype}, {@code false}
   *         otherwise.
   */
   ...
   * <b>ATTENTION:</b><br>
   * This is NOT an implementation of JSR 303 ({@code javax.validation}) nor is it strictly compatible to it. Instead
   * this is an abstraction that may be implemented using JSR 303 or maybe not. However, we want to be GWT compatible
   * and we want to directly bind the validation to the UI widgets (see
   * {@code UiWidgetWithValue#addValidator(ValueValidator)}). The API of JSR 303 is unfortunately biased. A simple
   * bridge to JSR 303 can be achieved via {@code javax.validation.Validator.validateProperty(Object, String, Class)}.
   *
   ...
   * E.g. {@code setPropertyValue(pojo, "foo", value)} should have the same effect as {@code pojo.setFoo(value)}. <br>
   *
   * @param pojo is the {@link net.sf.mmm.util.pojo.api.Pojo} owning the property.
   * @param propertyName is the {@link net.sf.mmm.util.pojo.descriptor.api.attribute.PojoAttributeName#getName() name}
   *        of the property.
