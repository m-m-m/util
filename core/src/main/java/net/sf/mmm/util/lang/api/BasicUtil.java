...
public interface BasicUtil {
   ...
   * to {@link Object#equals(Object)} the objects may be {@code null}.
   *
   ...
   * @return {@code true} if both objects are {@code null} or the first is NOT {@code null} and
   *         {@code o1.equals(o2)}, {@code false} otherwise.
   */
  boolean isEqual(Object o1, Object o2);

  /**
   * This method checks if two given arrays are structurally {@link Object#equals(Object) equal} to each
   * other. On arrays the {@link Object#equals(Object) equals}-method only checks for identity. This method
   * checks that both arrays have the same {@code length} and if so that the objects contained in the arrays
   * are {@link #isEqual(Object, Object) equal}. Additionally the given arrays may be {@code null} .
   *
   * @param array1 the first array to compare. It may be {@code null}.
   * @param array2 the second array to compare. It may be {@code null}.
   * @return {@code true} if both arrays are {@code null} or the first is NOT {@code null} and both arrays
   *         have the same {@code length} and {@link #isEqual(Object, Object) equal} content, {@code false}
   *         otherwise.
   */
  ...
  /**
   * This method checks if two given objects are {@link Object#equals(Object) equal} to each other. In advance
   * to {@link Object#equals(Object)} the objects may be {@code null} and arrays are
  ...
  /**
   * This method returns the index of the first element in the given {@code array} that equals (see
   * {@code checkEqual}) to the given {@code value}.
   *
   * @param value is the value to check. It may be {@code null}.
   * @param array is the array to check.
   * @param checkEqual - if {@code true} the {@code value} only needs to be {@link #isEqual(Object, Object)
   *        equal} to an element, if {@code false} it needs to be the same ({@code ==}).
  ...
  /**
   * This method returns the index of the first element in the given {@code array} that equals (see
   * {@code checkEqual}) to the given {@code value}. <br>
   * Unlike {@link #findInArray(Object, Object[], boolean)} this method also works for
   * {@link Class#isPrimitive() primitive} arrays (such as {@code int[]}). In such case you need to supply the
   * according {@link net.sf.mmm.util.reflect.base.ReflectionUtilImpl#getNonPrimitiveType(Class) object-type}
   * for {@code value}.
   *
   * @param value is the value to check. It may be {@code null}.
   * @param array is the array to check.
  ...
  /**
   * This method determines if the given {@code array} contains an element that equals (see {@code checkEqual}
   ...
   * @return {@code true} if both {@link CharIterator}s produced the same {@link CharIterator#next() next}
   *         chars until both ended at the same time, {@code false} otherwise.
   */
