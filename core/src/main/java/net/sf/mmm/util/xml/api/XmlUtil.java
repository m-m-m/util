...
public interface XmlUtil {
   ...
   * @param escapeQuotations if {@code true} also the ASCII quotation characters (apos {@code '\''}
   *        and quot {@code '"'}) will be escaped, else if {@code false} quotations are untouched.
   *        Set this to {@code true} if you are writing the value of an attribute.
   * @return the escaped string.
   */
  String escapeXml(String string, boolean escapeQuotations);

  /**
   * This method writes the given {@code string} to the {@code writer} while escaping special
  ...
  /**
   * This method creates a {@link Reader} from the given {@code inputStream} that uses the encoding
   ...
   * @return a reader on the given {@code inputStream} that takes respect on the encoding specified in
   ...
   *        are case-sensitive.
   * @return the value of the entity or {@code null} if no entity exists for the given
   *         {@code entityName}.
   */
   ...
   * {@code buffer}. This includes removing tags, un-escaping entities and parsing CDATA sections. Unlike
   ...
   * @param parserState is the state to continue on a subsequent call for multiple {@code htmlFragment}s
   *        of the same HTML-document or {@code null} for a fresh start.
   * @return the state at the end of {@code htmlFragment}. You can pass this as {@code parserState}
