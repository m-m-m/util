...
/**
 * This is the abstract base class for a simplified {@link Map} that associates elements ({@literal <E>}) with {@link Class} objects.
 * 
...
public abstract class AbstractClassHierarchyMap<E> {
   ...
   * {@code type} or a sub-type.
   * 
   * @param type is the {@link Class} for which the associated element is requested.
   * @return the element for the given type or {@code null} if no element is {@link #put(Class, Object)
   *         registered} for the given {@code type}.
   */
   ...
   * {@code currentType} in preference to the element {@code existing} that is already registered
   ...
   * @return {@code true} if the given {@code element} is preferable and should replace
   *         {@code existing} for {@code currentType}, {@code false} otherwise (if
   *         {@code existing} should remain {@link #get(Class) associated} with {@code currentType}
   *         ).
   */
  ...
  /**
   * This method registers the given {@code element} for the given {@code type}. Unlike a regular
   * {@link Map} this method will also recursively put the given {@code element} for all
   ...
   * @return the element that was associated directly with the given {@code type} and has NOW been
   *         replaced with {@code element}.
   */
   ...
   * @return {@code true} if the given {@code type} should be accepted, {@code false} to ignore
   *         {@code type}.
   */
