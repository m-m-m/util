...
/**
 * This is the interface for a collection of utility functions to deal with exceptions ({@link Throwable}s). It is
 * especially useful for {@link #convertForClient(Throwable) converting exceptions at application barriers}, e.g. to
 * prevent violating the OWASP principle <em>sensitive data exposure</em>.
 *
...
public interface ExceptionUtil extends ExceptionUtilLimited, Security {
   ...
   * In {@link net.sf.mmm.util.lang.api.EnvironmentDetector#isDevelopmentEnvironment() development environment} and
   * {@link net.sf.mmm.util.lang.api.EnvironmentDetector#ENVIRONMENT_TYPE_TEST test environment} the details should be
   * retained to support debugging. However, in
   * {@link net.sf.mmm.util.lang.api.EnvironmentDetector#isEnvironmentCloseToProduction() environments close to
   * production}, the details shall be removed.</li>
   * <li>Exceptions are {@link #convertForUser(Throwable) converted for the end-user} so that confusion with technical
   * details and exposure of internals in messages is avoided.</li>
   * <li>Potentially exceptions are {@link #convertForSerialization(Throwable, ExceptionTruncation) converted for
   * serialization}.</li>
   * </ul>
   * <br>
   * <b>ATTENTION:</b><br>
   * It is not the task of this utility to log exceptions. Instead this shall be done before this method is invoked. To
   * make use of the features of {@link net.sf.mmm.util.exception.api.NlsThrowable} such as the
   * {@link net.sf.mmm.util.exception.api.NlsThrowable#getUuid() UUID} that is logged and transferred to the client, you
   * should call {@link #convertForUser(Throwable)} in advance.
   *
