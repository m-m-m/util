...
public interface CharStreamScanner {
   ...
   *
   * @return {@code true} if there is at least one character available, {@code false} if the end has been reached.
   */
  boolean hasNext();

  /**
   * This method reads the current character and increments the {@link #getCurrentIndex() index} stepping to the next
   * character. You need to {@link #hasNext() check} if a character is available before calling this method.
   *
  ...
  /**
   * Like {@link #next()} this method reads the {@link #getCurrentIndex() current} character and increments the
   * {@link #getCurrentIndex() index}. If there is no character {@link #hasNext() available} this method will do nothing
   * and returns {@code 0} (the NULL character and NOT {@code '0'}).
   *
  ...
  /**
   * This method reads the current character without incrementing the {@link #getCurrentIndex() index}. You need to
   ...
   * {@link #getCurrentIndex() index}. If there is no character {@link #hasNext() available} this method will return
   * {@code 0} (the NULL character and NOT {@code '0'}).
   *
   * @return the current character or {@code 0} if none is {@link #hasNext() available}.
   */
  char forcePeek();
  ...
  /**
   * This method reads the long starting at the {@link #getCurrentIndex() current position} by reading as many Latin
   * digits as available but at maximum the given {@code maxDigits} and returns its {@link Long#parseLong(String)
   * parsed} value. <br>
   * <b>ATTENTION:</b><br>
   ...
   * @throws NumberFormatException if the current {@link #getCurrentIndex() current position} does NOT point to a
   *         number.
   */
  long readLong(int maxDigits) throws NumberFormatException;

  /**
   * This method reads the double value (decimal number) starting at the {@link #getCurrentIndex() current position} by
   * reading as many matching characters as available and returns its {@link Double#parseDouble(String) parsed} value.
   * <br>
   *
   * @return the parsed number.
   * @throws NumberFormatException if the current {@link #getCurrentIndex() current position} does NOT point to a
  ...
  /**
   * This method reads the float value (decimal number) starting at the {@link #getCurrentIndex() current position} by
   * reading as many matching characters as available and returns its {@link Float#parseFloat(String) parsed} value.
   * <br>
   *
   * @return the parsed number.
   * @throws NumberFormatException if the current {@link #getCurrentIndex() current position} does NOT point to a
  ...
  /**
   * This method acts as {@link #expect(String, boolean)} but if the expected String is NOT completely present, no
   * character is {@link #next() consumed} and the state of the scanner remains unchanged.
   *
   * @param expected is the expected string.
   * @param ignoreCase - if {@code true} the case of the characters is ignored when compared.
   * @return {@code true} if the {@code expected} string was successfully consumed from this scanner, {@code false}
   ...
   * <br>
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character after
   * the (first) {@code stop} character or to the end if NO such character exists.
   *
   * @param stop is the character to read until.
   ...
   *         {@code stop} character and {@code acceptEof} is {@code false}.
   */
   ...
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character after
   * the first {@link CharFilter#accept(char) accepted} character or to the end if NO such character exists and
   * {@code acceptEof} is {@code true}. Otherwise, if {@code null} is returned, the {@link #getCurrentIndex() current
   ...
   *         {@code filter} character or {@code null} if there was no {@link CharFilter#accept(char) accepted} character
   *         and {@code acceptEof} is {@code false}.
   * @since 7.0.0
   */
   ...
   * the input-string by adding the given {@code escape} character. After the call of this method, the
   * {@link #getCurrentIndex() current index} will point to the next character after the (first) {@code stop} character
   * or to the end if NO such character exists. <br>
   ...
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character after
   * the (first) {@code stop} character or to the end of the string if NO such character exists.
   *
   * @param stop is the character to read until.
   * @param acceptEof if {@code true} EOF will be treated as {@code stop}, too.
   ...
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character that
   * was NOT {@link CharFilter#accept(char) accepted} by the given {@code filter} or to the end if NO such character
   * exists.
   *
   ...
   * given {@code filter}. <br>
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character that
   * was NOT {@link CharFilter#accept(char) accepted} by the given {@code filter}. If the next {@code max} characters or
   * the characters left until the {@link #hasNext() end} of this scanner are {@link CharFilter#accept(char) accepted},
   * only that amount of characters are skipped.
   *
   * @see #skipWhile(char)
   *
   * @param filter is used to {@link CharFilter#accept(char) decide} which characters should be accepted.
   ...
   * After the call of this method, the {@link #getCurrentIndex() current index} will point to the next character after
   * the first occurrence of {@code substring} or to the end of the string if the given {@code substring} was NOT found.
   * <br>
   *
   * @param substring is the substring to search and skip over starting at the {@link #getCurrentIndex() current index}.
  ...
  /**
   * This method reads all {@link #next() next characters} until the given {@code substring} has been detected. <br>
   * If a {@link CharFilter#accept(char) stop character} is detected by the given {@code stopFilter} this method returns
   * {@code false} pointing to the character next to that stop character. Otherwise after this method, the
   * {@link #getCurrentIndex() current index} will point to the next character after the first occurrence of
   ...
   * E.g. use {@link #skipWhile(char) readWhile(' ')} to skip all blanks from the {@link #getCurrentIndex() current
   * index}. After the call of this method, the {@link #getCurrentIndex() current index} will point to the next
   * character that is different to {@code c} or to the end if NO such character exists.
   *
