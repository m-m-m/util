...
public interface CollectionReflectionUtil {
  ...
  /**
   * This method gets the size of the given {@code arrayMapOrCollection}. If {@code arrayMapOrCollection} is an array,
   * then its {@link java.lang.reflect.Array#getLength(Object) length} is returned. If it is a {@link java.util.Map} or
   * {@link Collection}, its {@link Collection#size() size} is returned.
   *
   ...
   * @throws NlsIllegalArgumentException if the given {@code arrayMapOrCollection} is invalid ( {@code null} or neither
   *         array nor {@link java.util.Map} or {@link Collection}).
   */
   ...
   * @return the item at the given {@code index}. May be {@code null} if the item itself is {@code null} or the index is
   *         greater than the {@link #getSize(Object) size} or {@code arrayOrCollection}.
   * @throws NlsIllegalArgumentException if the given {@code arrayOrList} is invalid ({@code null} or neither array nor
   *         {@link java.util.List}).
   */
   ...
   *        {@link #getSize(Object) size} of {@code arrayOrCollection} an {@link IndexOutOfBoundsException} will be
   *        thrown. Else if {@code true}, {@code null} is returned in this case.
   * @return the item at the given {@code index}. May be {@code null} if the item itself is {@code null} or the index is
   *         greater or equal to the {@link #getSize(Object) size} of {@code arrayOrCollection}.
   * @throws NlsIllegalArgumentException if the given {@code arrayOrList} is invalid ({@code null} or neither array nor
   *         {@link java.util.List}).
   */
  ...
  /**
   * This method sets the given {@code item} at the given {@code index} in {@code arrayOrCollection}. It uses a default
   * value for {@code maximumGrowth} and no {@code arrayReceiver} ( {@code null}).
   *
   ...
   * @return the item at position {@code index} in {@code arrayOrList} that has been replaced by {@code item}. This can
   *         be {@code null}. Additional if the {@code arrayOrList} has been increased, {@code null} is returned.
   * @throws NlsIllegalArgumentException if the given {@code arrayOrList} is invalid ({@code null} or neither array nor
   *         {@link java.util.List}).
   */
   ...
   * {@code index + 1} by {@link java.util.List#add(Object) adding} {@code null} values so the {@code item} can be set.
   * It uses a default value for {@code maximumGrowth}.
   *
   ...
   *        {@link System#arraycopy(Object, int, Object, int, int) array-copy} of {@code arrayOrList} with an increased
   *        {@link java.lang.reflect.Array#getLength(Object) length}. It can be {@code null} to disable array-copying.
   * @return the item at position {@code index} in {@code arrayOrList} that has been replaced by {@code item}. This can
   *         be {@code null}. Additional if the {@code arrayOrList} has been increased, {@code null} is returned.
   * @throws NlsIllegalArgumentException if the given {@code arrayOrList} is invalid ({@code null} or neither array nor
   *         {@link java.util.List}).
   ...
   * {@code index}, the {@link java.util.List#size() size} of the {@link java.util.List} will be increased to
   * {@code index + 1} by {@link java.util.List#add(Object) adding} {@code null} values so the {@code item} can be set.
   * However the number of {@link java.util.List#add(Object) adds} is limited to {@code maximumGrowth}.
   *
   * @see java.util.List#set(int, Object)
   *
   * @param arrayOrList is the array or {@link java.util.List}.
   * @param index is the position where to set the item.
   * @param item is the item to set.
   * @param maximumGrowth is the maximum number by which the {@link #getSize(Object) size} of {@code arrayOrList} will
   *        be increased (with {@code null} values) to reach {@code index + 1} so the {@code item} can be set. Set this
   *        value to {@code 0} to turn off this feature (and leave the {@link #getSize(Object) size} untouched). Please
   *        always specify a real maximum ({@literal <=65536}) and do NOT use {@link Integer#MAX_VALUE} since this might
   *        cause memory holes if something goes wrong. If {@code arrayOrList} is an array, increasing can only happen
   *        by creating a new array. To receive such new array, you need to supply an {@code arrayReceiver}. Otherwise
   *        (if {@code null}) this method behaves for arrays as if {@code maximumGrowth} was {@code 0}. If an array
   *        should be increased, a new array with the size of {@code index + 1} is created. The original items are
   *        {@link System#arraycopy(Object, int, Object, int, int) copied}, the given {@code item} is set on the copy
   *        instead while the original array remains unchanged. Then the new array is
   *        {@link GenericBean#setValue(Object) set} to the {@code arrayReceiver}.
   * @param arrayReceiver is a {@link GenericBean} that allows to receive an
   *        {@link System#arraycopy(Object, int, Object, int, int) array-copy} of {@code arrayOrList} with an increased
   *        {@link java.lang.reflect.Array#getLength(Object) length}. It can be {@code null} to disable array-copying.
   * @return the item at position {@code index} in {@code arrayOrList} that has been replaced by {@code item}. This can
   ...
   * @return the given {@code arrayOrCollection} if it was a {@link Collection}. Otherwise, in case of an array, a new
   *         array with a {@link java.lang.reflect.Array#getLength(Object) length} increased by {@code 1} and the
   *         elements of {@code arrayOrCollection} appended with the given {@code item} is returned.
   */
   ...
   *         {@code arrayOrCollection} if it was a {@link Collection} and the {@code item} has been removed. Otherwise,
   *         in case of an array, a new array with a {@link java.lang.reflect.Array#getLength(Object) length} decreased
   *         by {@code 1} and the elements of {@code arrayOrCollection} without the first occurrence of the given
   *         {@code item} is returned.
   */
