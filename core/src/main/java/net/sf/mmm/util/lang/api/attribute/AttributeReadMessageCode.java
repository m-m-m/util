...
public abstract interface AttributeReadMessageCode {
   ...
   * {@link AttributeReadUuid#getUuid() UUID} or {@link AttributeReadId#getId() ID} are typically unique per instance of
   * an object the code is unique for all instances of the exact same kind. So e.g. a particular kind of
   * {@link net.sf.mmm.util.exception.api.NlsThrowable exception} or {@code ValidationFailure} can be identified by its
   ...
   * {@link String} literal). This code may be used as a compact identifier to reference the related problem or
   * information as well as for automatic tests of error situations that should remain stable even if the message text
   * gets improved or the locale is unknown.
   *
