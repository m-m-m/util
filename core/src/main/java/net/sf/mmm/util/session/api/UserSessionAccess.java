...
public final class UserSessionAccess {
  ...
  private static void setProvider(UserSessionProvider sessionProvder) {
    ...
    if (provider != null) {
      LoggerFactory.getLogger(UserSessionAccess.class).warn(
          "ATTENTION: replacing existing UserSessionProvider {} with {}!", provider, sessionProvder);
    }
    synchronized (UserSessionAccess.class) {
   ...
   */
  public abstract static class AbstractUserSessionProvider extends AbstractLoggableComponent implements
      UserSessionProvider {
