 ...
 * abstract from the implementation of the component so it gets replaceable and the design is maintainable.
 * <br>
 * The most important type in Java to define an API is the {@link Class#isInterface() interface}. However an
 * API may also consist of {@link Enum} types or POJO {@link Class classes} (see
 * {@code net.sf.mmm.util.pojo.api.Pojo}) and {@link net.sf.mmm.util.lang.api.Datatype}s that are accepted as
 * arguments by an interface. The actual <em>API</em> of a
 * {@link net.sf.mmm.util.component.api.ComponentSpecification component} is therefore the combination of all
 * these types. It has to be <em>self-contained</em> what means that it only contains references to types
 * legally considered as part of the <em>API</em>. With references we are NOT talking about JavaDoc-links.
 * <br>
...
public interface Api {
   ...
   * siblings of the {@code api} packages. Typically that {@link Class} is named like the interface with the
   * prefix {@code Abstract}. You are strongly encouraged to extend these base implementations rather than
   * directly implementing the interface to gain compatibility in case of an extension. The latter is still
   * allowed but you have to be aware of the consequences that you may have to fix compile errors after
