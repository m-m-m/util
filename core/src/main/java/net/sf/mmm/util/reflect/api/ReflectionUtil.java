...
public interface ReflectionUtil extends ReflectionUtilLimited {
   ...
   *         {@link Object#getClass() class} of the object from the given array at the same position or {@code null}, if
   *         that object is {@code null}.
   */
   ...
   * @throws TypeNotFoundException if a class could NOT be found (e.g. in {@code java.util.Map<java.long.String>} - what
   *         should be {@code lang} instead of {@code long}).
   * @throws NlsIllegalArgumentException if the given {@code type} could NOT be parsed (e.g.
  ...
  /**
   * This method determines if the given {@code interfaceClass} is a marker-interface (e.g. {@link java.io.Serializable}
   * or {@link Cloneable}). A marker-interface is also called a tagging-interface.
   *
   * @param interfaceClass is the {@link Class} reflecting the interface to check.
   * @return {@code true} if the given {@code interfaceClass} is a marker-interface, {@code false} otherwise (if regular
   ...
   * @param exactTypeMatch - if {@code true}, the {@code fieldType} must match exactly the type of the static field,
   *        else if {@code false} the type of the field may be a sub-type of {@code fieldType} or one of the types may
   *        be {@link Class#isPrimitive() primitive} while the other is the {@link #getNonPrimitiveType(Class)
   *        according} object-type.
   * @param mustBeFinal - if {@code true}, an {@link IllegalArgumentException} is thrown if the specified static field
   *        exists but is NOT {@link java.lang.reflect.Modifier#isFinal(int) final}, {@code false} otherwise.
   * @param inherit if {@code true} the field may be inherited from a {@link Class#getSuperclass() super-class} or
   *        {@link Class#getInterfaces() super-interface} of {@code type}, else if {@code false} the field is only
   *        accepted if it is declared in {@code type}.
  ...
  /**
   * This method gets the parent method of the given {@code method}. The parent method is the method overridden (is the
   * sense of {@link Override}) by the given {@code method} or directly inherited from an {@link Class#getInterfaces()
   * interface}.
   *
  ...
  /**
   * This method finds all classes that are located in the package identified by the given {@code packageName}. <br>
   * <b>ATTENTION:</b><br>
   ...
   * {@code qualifiedClassNames} and returns them as {@link Set}.
   *
  ...
  /**
   * This method loads the classes given as {@link Collection} of {@link Class#getName() fully qualified names} by
   * {@code qualifiedClassNames}. It returns a {@link Set} containing only those loaded classes that are
  ...
  /**
   * This method finds all resources that are located in the package identified by the given {@code packageName}. <br>
   * <b>ATTENTION:</b><br>
   * This is a relative expensive operation. Depending on your classpath multiple directories, JAR-, and WAR-files may
   * need to be scanned.
   *
