...
public enum Weekday implements SimpleDatatype<String> {
  ...
  /**
   * This method gets the ID constant of {@link java.util.Calendar} representing this {@link Weekday}. This is the
   * Christian representation where {@link #SUNDAY} is the first day of the week represented with 1. The enum constants
   * of {@link Weekday} are ordered such that {@link #MONDAY} comes first, so you can use {@link #ordinal()} for other
   * representation. Please note that {@link #ordinal()} is {@code 0} based so you might want to increment the result.
   *
  ...
  /**
   * This method returns the {@link Weekday} resulting if the given number of {@code days} are elapsed after the current
   * {@link Weekday} (this). The {@link Weekday} will wrap so {@link #add(int) add(7)} will return the {@link Weekday}
   * itself (this) just like {@link #add(int) add(0)} or e.g. {@link #add(int) add(-14)}.
   *
   * @param days are the number of days to add. May be negative to subtract or zero ({@code 0}) for no change.
   ...
   *
   * @param calendar is the {@link Calendar}.
   * @return the {@link Weekday} representing the {@link Calendar#DAY_OF_WEEK weekday} of the given {@link Calendar}.
   */
   ...
   * @return the {@link Weekday} representing the {@link #getCalendarId() calendar ID} or {@code null} if no such
   *         {@link Weekday} exists (illegal {@code calendarId}).
   */
