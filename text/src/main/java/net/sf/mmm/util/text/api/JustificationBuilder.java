 ...
 * <td>{@code «mode»}</td>
 * <td>{@code [|]}</td>
 * <td>if the mode is truncate(|) then the string will be truncated if its length is greater than «with» so
 * the result will always have the length of «with». Please note that truncate can remove valuable information
 * or cause wrong results (e.g. "10000" with a justification of " +3|" will result in "100").</td>
 * </tr>
 * </table>
 *
 * Examples:
