...
public interface UnicodeUtil {
  ...
  /**
   * The small <em>schwa</em> letter is used in the International Phonetic Alphabet to represent the "mid
   * central unrounded vowel". It is used for instance in the second syllable of the English word "harmony".
   * This letter uses the same glyph as {@link #LATIN_SMALL_LETTER_TURNED_E}.
   */
   ...
   * @return a sequence of characters with the transliteration of the given character or {@code null} if no
   *         such transliteration is available.
   */
  String transliterate(char character);

  /**
   * This method gets the <a href="http://en.wikipedia.org/wiki/Transliteration">transliteration</a> of the
   * given {@code text}. This method will support common transliteration standards such as ISO 843 (greek),
   * ISO 9 (cyrillic), etc. However, for some writing systems there are multiple ways of transliteration and
   * some things done by this method may not officially be called transliteration. So please consider it as a
   * pragmatic way to convert text to the Latin alphabet. We are looking for help in supporting additional
   * writing systems but not for scientific discussion about linguistic.
   *
   * @see java.text.Normalizer
   *
  ...
  /**
   * This method converts the given {@code text} to the best possible ASCII-representation. All
   ...
   * This method converts the given {@code text} to the best possible ASCII-representation. All
   * ASCII-characters will remain unchanged. All other characters are {@link #normalize2Ascii(char) normalized
   * to ASCII} and if not possible replaced by {@code nonNormalizableCharaterReplacement}.
   *
   * @see java.text.Normalizer
   *
   * @param text is the text to convert.
