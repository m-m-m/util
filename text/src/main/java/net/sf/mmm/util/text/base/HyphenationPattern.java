 ...
 * The concept is based on the thesis <em>Word Hy-phen-a-tion by Com-put-er</em> by <em>Franklin Mark
 * Liang</em>. Out of an entire dictionary of hyphenated words for a specific language, a set of
 * {@link HyphenationPattern patterns} is extracted. To allow correct results with a reasonable small set of
 * patterns, these patterns form a chain of positive rules and exceptions. Therefore a pattern can
 * {@link HyphenationPatternPosition#ranking rank} a potential hyphenation-position with a number from
 * {@code 1} to {@code 9}. If two patterns apply for a hyphenation-position the higher number wins. Odd
 * numbers indicate a hyphenation while even values indicate an exception that should NOT be hyphenated. The
 * character '.' is used at the beginning and/or end of a pattern to indicate that it should only match at the
 * beginning/end of the word to hyphenate. <br>
 ...
 * The string {@code "Computer"} will be transformed to {@code ".computer."} that matches the following
 * patterns:
 * <ul>
 * <li>4m1p</li>
 * <li>pu2t</li>
 ...
 * <li>put3er</li>
 * </ul>
 * This results to {@code co4m5pu2t3er} so the hyphenated input String is finally <code>"Com-put-er"</code>.
 * The challenge is to implement this algorithm in an efficient way.
 *
