...
public class UnicodeUtilImpl extends AbstractComponent implements UnicodeUtil {
    ...
    for (DiacriticalMark mark : DiacriticalMark.values()) {
      for (char composed : mark.getComposedCharacters()) {
        if (!CHARACTER_TO_ASCII_MAP.containsKey(composed)) {
          ...
          if (decomposed == null) {
            LoggerFactory.getLogger(UnicodeUtilImpl.class).error("Illegal diacritic '" + mark + "' could NOT decomposed '" + composed + "'!");
          } else {
  ...
  private static void initIso9(Map<Character, String> map) {
    ...
    map.put(CYRILLIC_CAPITAL_LETTER_ABKHASIAN_CHE_WITH_DESCENDER, Character.toString(LATIN_CAPITAL_LETTER_C_WITH_CEDILLA) + COMBINING_BREVE);
    ...
    map.put(CYRILLIC_CAPITAL_LETTER_ZHE_WITH_DESCENDER, Character.toString(LATIN_CAPITAL_LETTER_Z_WITH_CARON) + COMBINING_CEDILLA);
    ...
    map.put(CYRILLIC_SMALL_LETTER_ABKHASIAN_CHE_WITH_DESCENDER, Character.toString(LATIN_SMALL_LETTER_C_WITH_CEDILLA) + COMBINING_BREVE);
    ...
    map.put(CYRILLIC_SMALL_LETTER_ZHE_WITH_DESCENDER, Character.toString(LATIN_SMALL_LETTER_Z_WITH_CARON) + COMBINING_CEDILLA);
