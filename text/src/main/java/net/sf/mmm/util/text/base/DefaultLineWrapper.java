...
public class DefaultLineWrapper extends AbstractLineWrapper {
  ...
  protected void autoAdjustWidthOfColumns(ColumnState[] columnStates, TextTableInfo tableInfo) {
    ...
    if (widthRemaining < autoAdjustColumnCount) {
      ...
      throw new IllegalArgumentException(
          "Table width (" + tableWidth + ") may not be less than static width + auto adjust column count (" + (staticTableWidth + autoAdjustColumnCount) + ")");
    }
  ...
  protected static class ColumnState extends TextColumn {
    ...
    public ColumnState(String text, TextColumnInfo columnInfo, HyphenatorBuilder hyphenatorBuilder) {
      ...
      if ((columnInfo.getIndent() == null)
          || ((columnInfo.getWidth() != TextColumnInfo.WIDTH_AUTO_ADJUST) && (columnInfo.getIndent().length() >= columnInfo.getWidth()))) {
     ...
     *
     * @return the next {@link TextSegment} or {@code null} if NOT available ( {@link #getCurrentSegment()
     *         current segment} is the last segment or also {@code null}).
     */
     ...
     * segment. Otherwise it will always return {@code 0}.
     *
     * @return the subsequentNewlineCount the number of subsequent {@link TextSegmentType#NEWLINE} segments
     *         including the {@link #getCurrentSegment() current segment} or {@code 0} if the
     ...
     * @return the {@link TextSegment} with the next segment-data or {@code null} if the {@link #getText()
     *         text} is completed.
     */
  ...
  protected static final class AutoAdjustInfo implements Comparable<AutoAdjustInfo> {
    ...
    public int hashCode() {
      ...
      final int prime = 31;
      int result = 1;
      result = prime * result + ((this.columnState == null) ? 0 : this.columnState.hashCode());
      result = prime * result + this.lineCount;
      result = prime * result + this.lineLengthMax;
      return result;
    }
    ...
    public boolean equals(Object obj) {
      ...
      if (this.columnState == null) {
        if (other.columnState != null) {
          return false;
        }
      } else if (!this.columnState.equals(other.columnState)) {
        return false;
      }
      if (this.lineCount != other.lineCount) {
        return false;
      }
      if (this.lineLengthMax != other.lineLengthMax) {
        return false;
      }
      return true;
    }
