...
public class DefaultLineWrapperTest extends Assertions {
    ...
    for (int i = 0; i < columnStates.length; i++) {
      ...
      if (TextColumnInfo.WIDTH_AUTO_ADJUST == columnStates[i].width) {
        if (columns.length == 1) {
          ...
          int indexBorderRight = wrappedText.subSequence(currentWidth, tableWidth).toString().indexOf(columnInfo.getBorderRight());
        ...
        if (newline == 0) {
          ...
          switch (columnInfo.getIndentationMode()) {
            ...
            case NO_INDENT_AFTER_DOUBLE_NEWLINE:
              ...
              break;
            default :
              throw new IllegalStateException("" + columnInfo.getIndentationMode());
          }
        }
        state.indent = indent;
        // right border
  ...
  public void testMultiColumn() {
    ...
    text = "This is a very long sentence without any newlines at all and with the "
        + "extraordinary long, strange and undefined and therefore un-hyphenate-able " + "word hushgigloobulathifagomasiwashutumiginatorgobonodile.";
