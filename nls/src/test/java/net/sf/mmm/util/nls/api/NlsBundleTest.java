...
public class NlsBundleTest extends Assertions {
  ...
  public void testNlsBundleObjectMethods() {
    ...
    // given
    Class<NlsBundleUtilCoreTestRoot> bundleInterface = NlsBundleUtilCoreTestRoot.class;
    NlsBundleUtilCoreTestRoot bundle = getBundleFactory().createBundle(bundleInterface);

    // then
    assertThat(bundle.toString()).isEqualTo("net.sf.mmm.util.nls.NlsBundleUtilCoreTest");
  ...
  private void verifyErrorObjectNotFound(String object, String key, NlsMessage errorObjectNotFound) {
    ...
    Assert.assertEquals("Object \"" + object + "\" zu \"" + key + "\" konnte nicht gefunden werden!", errorObjectNotFound.getLocalizedMessage(Locale.GERMAN));
  }
