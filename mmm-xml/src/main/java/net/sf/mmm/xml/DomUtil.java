/* $Id: DomUtil.java 191 2006-07-24 21:00:49Z hohwille $ */
...
public final class DomUtil {
     ...
     * 
     * @param indent -
     *        <code>true</code> if the XML should be indented (automatically
     *        add linebreaks before opening tags), <code>false</code>
     *        otherwise.
     ...
     * @return the first element in the node-list or <code>null</code>, if
     *         the node-list contains NO element.
     */
     ...
     *         all if tag-name is <code>null</code>). If no such element
     *         exists in the node-list, <code>null</code> is returned.
     */
     ...
     *        is the xml node containing the text.
     * @param depth
     *        specifies how deep to step into child elements. If less than 1,
     *        the text of child elements is ignored.
     ...
     * element including the text of CDATA sections. The text of child elements
     * is only appended according to the given depth. If the depth is less than
     * 1, child elements are ignored, if equal to 1, the text of child elements
     * is included without their child elements. For an infinte depth use
     ...
     * @throws XmlException
     *         if the input stream produced an IOException or the XML is
     *         invalid.
     */
     ...
     *         whatever).
     */
    public static void transformXml(Source source, Result result, boolean indent)
            throws XmlException {
    ...
    public static void toXml(XmlWriterIF xmlWriter, Node node) throws XmlException {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (prefix == null) {
                ...
                xmlWriter.writeStartElement(prefix, element.getLocalName(), element
                        .getNamespaceURI());
            }
            ...
            if (prefix == null) {
                ...
                if (prefix.equals(XmlUtil.NAMESPACE_PREFIX_XMLNS)) {
                    xmlWriter.writeNamespaceDeclaration(attribute.getLocalName(), attribute
                            .getValue());
                } else {
                    xmlWriter
                            .writeAttribute(attribute.getLocalName(), attribute.getValue(), prefix);
                }
            }
